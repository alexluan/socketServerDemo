        ...
        if fd > 0 {
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0)) { [weak self] in
                while self != nil {
                    ...
                    var port:Int32 = 0
                    let clientfd: Int32 = c_ytcpsocket_accept(self!.fd!, ip: &buff, port: &port)//调用ytcpsocket.c语言函数，socket.swift中重新命名c语言函数
                    ...
                    if clientfd > 0 {
                        ...
                        let tcpClient: TCPClient = TCPClient(addr: addr, port: Int(port))
